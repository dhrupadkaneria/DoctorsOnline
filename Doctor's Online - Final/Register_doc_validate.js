...
function timeVal(fieldElem,a)
{
	...
	if(a<=0)
	{
		...
		fieldElem.next().removeClass().addClass("error");
		fieldElem.next().text("End time Should be greater than Start time")
	}
	else
	{
			fieldElem.next().removeClass();
...
function ccVal(fieldElem)
{
	if(fieldElem.val().length>0)
	{
		...
		if(val)
		{
			...
			fieldElem.next().removeClass();
				fieldElem.next().text("");
		if(fieldElem.val().length!=16)
		{
			fieldElem.next().removeClass().addClass("error");
			fieldElem.next().text("16 Digit Credit Card number is required");
		}
		}
		else
		{
			fieldElem.next().removeClass().addClass("error");
			fieldElem.next().text("Only Digits allowed for Credit Card Numbers");
		}
	}
	else
	{
...
$('<span class="info" style="VISIBILITY:hidden;display:none"></span>').insertAfter($('#location'));
$('#location').focusin(function(){	
	validateField($('#location'),infomsg6,cityVal)
});
...
$('<span class="info" style="VISIBILITY:hidden;display:none"></span>').insertAfter($('#cc_no'));
$('#cc_no').focusin(function(){	
	validateField($('#cc_no'),infomsg5,ccVal)
});


$('<span class="info" style="VISIBILITY:hidden;display:none"></span>').insertAfter($('#Ttime'));
$('#Ttime').focusout(function(){	
var a=$('#Ttime').val()-$('#Ftime').val();
